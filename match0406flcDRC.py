...
cat = np.genfromtxt(dir+'flcDRCpos0406r4.dat')
...
while (nF):
    matchrows = cat[(abs(master[row][x] - cat[:,xt1]) <= matchtol) & (abs(master[row][y] - cat[:,yt1]) <= matchtol)]
    ...
    if (len(matchrows) == 1):
      ...
      row = row + 1
         ...
         for dd in range(len(matchrows)):
             distDiff[dd] = np.sqrt( (master[row][x] - matchrows[dd][xt1])**2 +  (master[row][y] - matchrows[dd][yt1])**2)
...
#
np.savetxt(dir+'drcFLCidxr4.dat',master,header=header)

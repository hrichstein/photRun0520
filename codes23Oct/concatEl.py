...
def concat(name_arr,source,catDir='../forElena16Nov/'):

    for ff in range(len(name_arr)):

        if ff==0:
            f_out = np.genfromtxt(catDir + name_arr[ff] + '_cat16Nov.dat')
            fhead = np.genfromtxt(catDir + name_arr[ff] + '_cat16Nov.dat',
                                  names=True)
        else:
            f_temp = np.genfromtxt(catDir+ name_arr[ff] + '_cat16Nov.dat')
            f_out = np.vstack((f_out,f_temp))

    colNs = np.array(fhead.dtype.names)
    header = ' '.join(colNs)

    np.savetxt(catDir + source + '_cat16Nov.dat',f_out,header=header,
               fmt='%1.5f')
